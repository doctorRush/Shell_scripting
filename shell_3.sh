...
today_string_length=0
for item in "${today_array[@]}"
do
    if [[ today_string_length -lt ${#item} ]]
    then
        today_string_length=${#item}
    fi 
done

tomorrow_string_length=0

for item in "${tomorrow_array[@]}"
do
    if [[ tomorrow_string_length -lt ${#item} ]]
    then
        tomorrow_string_length=${#item}
    fi
done
...
print_dashes $((today_string_length+tomorrow_string_length+7))

echo -ne "| Today"
print_spaces $((today_string_length-5)) false

echo -ne " Tomorrow"
print_spaces $((tomorrow_string_length-8)) true
...
for (( j=0; j<highest_arr_length; j++ ))
    do 
        ...
        print_spaces $((today_string_length-item_length)) false

        echo -ne " "${tomorrow_array[j]}
        item_length=${#tomorrow_array[j]}
        print_spaces $((tomorrow_string_length-item_length)) true
    done
